...
namespace libmorton {
	...
	inline morton m2D_e_sLUT(const coord x, const coord y) {
		...
		for (unsigned int i = sizeof(coord); i > 0; --i) {
			...
			answer =
				answer << 16 |
				Morton2D_encode_y_256[(y >> shift) & EIGHT_BIT_MASK] |
				Morton2D_encode_x_256[(x >> shift) & EIGHT_BIT_MASK];
		}
		return answer;
	}
	...
	inline morton m2D_e_LUT(const coord x, const coord y) {
		...
		for (unsigned int i = sizeof(coord); i > 0; --i) {
			...
			answer =
				answer << 16 |
				(Morton2D_encode_x_256[(y >> shift) & EIGHT_BIT_MASK] << morton(1)) |
				(Morton2D_encode_x_256[(x >> shift) & EIGHT_BIT_MASK]);
		}
		return answer;
	}
	...
	inline morton compute2D_ET_LUT_encode(const coord c, const coord *LUT) {
		...
		unsigned int i = 0;
		while (maxbit >= i) {
			answer |= ((morton)LUT[(c >> i) & EIGHT_BIT_MASK]) << i * 2;
			i += 8;
		}
		return answer;
	}
	...
	inline coord morton2D_DecodeCoord_LUT256(const morton m, const uint_fast8_t *LUT, const unsigned int startshift) {
		...
		for (unsigned int i = 0; i < loops; ++i) {
			a |= ((morton)LUT[(m >> ((i * 8) + startshift)) & EIGHT_BIT_MASK] << (4 * i));
		}
	...
	inline void m2D_d_sLUT_ET(const morton m, coord& x, coord& y) {
		...
		while (firstbit_location > i) {
			morton m_shifted = (m >> i) & EIGHT_BIT_MASK;
	...
	inline void m2D_d_LUT_ET(const morton m, coord& x, coord& y) {
		...
		unsigned int shiftback = 0;
		while (firstbit_location > i) {
			x |= (coord)Morton2D_decode_x_256[(m >> i) & EIGHT_BIT_MASK] << shiftback;
			y |= (coord)Morton2D_decode_x_256[(m >> (i + 1)) & EIGHT_BIT_MASK] << shiftback;
			shiftback += 4;
			i += 8;
		}
	}

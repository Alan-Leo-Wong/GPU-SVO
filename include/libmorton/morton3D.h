...
namespace libmorton {
	...
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER morton m3D_e_sLUT_ET(const coord x, const coord y, const coord z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER morton m3D_e_LUT(const coord x, const coord y, const coord z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER morton m3D_e_LUT_ET(const coord x, const coord y, const coord z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER morton m3D_e_magicbits(const coord x, const coord y, const coord z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER morton m3D_e_for(const coord x, const coord y, const coord z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER morton m3D_e_for_ET(const coord x, const coord y, const coord z);

	// AVAILABLE METHODS FOR DECODING
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_sLUT(const morton m, coord& x, coord& y, coord& z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_sLUT_ET(const morton m, coord& x, coord& y, coord& z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_LUT(const morton m, coord& x, coord& y, coord& z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_LUT_ET(const morton m, coord& x, coord& y, coord& z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_magicbits(const morton m, coord& x, coord& y, coord& z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_for(const morton m, coord& x, coord& y, coord& z);
	template<typename morton, typename coord> inline CUDA_CALLABLE_MEMBER void m3D_d_for_ET(const morton m, coord& x, coord& y, coord& z);

	// ENCODE 3D Morton code : Pre-Shifted LookUpTable (sLUT)
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_sLUT(const coord x, const coord y, const coord z) {
		morton answer = 0;
		for (unsigned int i = sizeof(coord); i > 0; --i) {
			unsigned int shift = (i - 1) * 8;
			answer =
				answer << 24 |
				(Morton3D_encode_z_256[(z >> shift) & EIGHT_BIT_MASK] |
					Morton3D_encode_y_256[(y >> shift) & EIGHT_BIT_MASK] |
					Morton3D_encode_x_256[(x >> shift) & EIGHT_BIT_MASK]);
		}
		return answer;
	}
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_LUT(const coord x, const coord y, const coord z) {
		morton answer = 0;
		for (unsigned int i = sizeof(coord); i > 0; --i) {
			unsigned int shift = (i - 1) * 8;
			answer =
				answer << 24 |
				(Morton3D_encode_x_256[(z >> shift) & EIGHT_BIT_MASK] << morton(2)) |
				(Morton3D_encode_x_256[(y >> shift) & EIGHT_BIT_MASK] << morton(1)) |
				Morton3D_encode_x_256[(x >> shift) & EIGHT_BIT_MASK];
		}
		return answer;
	}
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton compute3D_ET_LUT_encode(const coord c, const coord *LUT) {
		...
		for (int i = (int)ceil((maxbit + 1) / 8.0f); i >= 0; --i) {
			...
			answer = answer << 24 | (LUT[(c >> shift) & EIGHT_BIT_MASK]);
		}
		return answer;
	}
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_sLUT_ET(const coord x, const coord y, const coord z) {
	...
	// Figuring this out is probably too costly in most cases.
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_LUT_ET(const coord x, const coord y, const coord z) {
		morton answer_x = compute3D_ET_LUT_encode<morton, coord>(x, Morton3D_encode_x_256);
	...
	static __constant__ uint_fast32_t magicbit3D_masks32_encode[6] = { 0x000003ff, 0, 0x30000ff, 0x0300f00f, 0x30c30c3, 0x9249249 }; // we add a 0 on position 1 in this array to use same code for 32-bit and 64-bit cases
	static __constant__ uint_fast64_t magicbit3D_masks64_encode[6] = { 0x1fffff, 0x1f00000000ffff, 0x1f0000ff0000ff, 0x100f00f00f00f00f, 0x10c30c30c30c30c3, 0x1249249249249249 };

	// HELPER METHOD: Magic bits encoding (helper method)
	template<typename morton, typename coord>
	static inline CUDA_CALLABLE_MEMBER morton morton3D_SplitBy3bits(const coord a) {
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_magicbits(const coord x, const coord y, const coord z) {
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_for(const coord x, const coord y, const coord z) {
		morton answer = 0;
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER morton m3D_e_for_ET(const coord x, const coord y, const coord z) {
		morton answer = 0;
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER coord morton3D_DecodeCoord_LUT256(const morton m, const uint_fast8_t *LUT, const unsigned int startshift) {
		morton a = 0;
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_sLUT(const morton m, coord& x, coord& y, coord& z) {
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_LUT(const morton m, coord& x, coord& y, coord& z) {
		x = morton3D_DecodeCoord_LUT256<morton, coord>(m, Morton3D_decode_x_512, 0);
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_sLUT_ET(const morton m, coord& x, coord& y, coord& z) {
		x = 0; y = 0; z = 0;
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_LUT_ET(const morton m, coord& x, coord& y, coord& z) {
		x = 0; y = 0; z = 0;
		unsigned long firstbit_location = 0;
		if (!findFirstSetBit<morton>(m, &firstbit_location)) { return; }
		unsigned int i = 0;
	...
	static __constant__ uint_fast32_t magicbit3D_masks32_decode[6] = { 0, 0x000003ff, 0x30000ff, 0x0300f00f, 0x30c30c3, 0x9249249 }; // we add a 0 on position 0 in this array to use same code for 32-bit and 64-bit cases
	static __constant__ uint_fast64_t magicbit3D_masks64_decode[6] = { 0x1fffff, 0x1f00000000ffff, 0x1f0000ff0000ff, 0x100f00f00f00f00f, 0x10c30c30c30c30c3, 0x1249249249249249 };

	// HELPER METHOD for Magic bits decoding
	template<typename morton, typename coord>
	static inline CUDA_CALLABLE_MEMBER coord morton3D_GetThirdBits(const morton m) {
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_magicbits(const morton m, coord& x, coord& y, coord& z) {
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_for(const morton m, coord& x, coord& y, coord& z) {
		x = 0; y = 0; z = 0;
		unsigned int checkbits = (sizeof(morton) * 8) / 3;
		for (unsigned int i = 0; i <= checkbits; ++i) {
			morton selector = 1;
	...
	template<typename morton, typename coord>
	inline CUDA_CALLABLE_MEMBER void m3D_d_for_ET(const morton m, coord& x, coord& y, coord& z) {
		x = 0; y = 0; z = 0;
		unsigned long firstbit_location = 0;
		if (!findFirstSetBit<morton>(m, &firstbit_location)) return;
